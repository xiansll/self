...
public:
	...
	void update_skin(bool force = true) {
		// Resolve C_CSWeaponBase::UpdateSkin by signature (virtual index 110 was
		// wrong for this build → skin written but never rebuilt). Signature from
		// a working reference for the same client.dll.
		static auto fn = reinterpret_cast<void(__fastcall*)(void*, int)>(
			g_opcodes->scan(g_modules->m_modules.client_dll.get_name(),
