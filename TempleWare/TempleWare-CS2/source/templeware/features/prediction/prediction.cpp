...
void C_Prediction::Start(CUserCmd* pCmd)
{
	...
	if (!g_ctx->local_pawn || !g_ctx->local_controller)
		return;

	C_PlayerMovementService* pMovementServices = g_ctx->local_pawn->m_pMovementServices();
	...
	C_PlayerMovementService* pMoveService = g_ctx->local_pawn->m_pMovementServices();
	if (!pMoveService)
		return;
    ...
    m_pred_data.m_nPlayerTickFraction = client_info->m_player_tick_fraction;
...
void C_Prediction::End(CUserCmd* pCmd) {
    ...
    *reinterpret_cast<int*>(reinterpret_cast<uintptr_t>(g_ctx->local_controller) + SchemaFinder::Get(hash_32_fnv1a_const("CBasePlayerController->m_nTickBase"))) = m_pred_data.m_nTickBase;
