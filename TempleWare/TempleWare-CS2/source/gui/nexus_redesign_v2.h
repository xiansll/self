...
// neXus redesign renderer — compatible with the project's Dear ImGui 1.75.
...
namespace NexusRedesign
{
    ...
    inline void RowSliderF(const char* label, float* v, float mn, float mx, const char* fmt)
    {
        ...
        ImGui::SliderFloat(id, v, mn, mx, "", 1.0f);
    }
    ...
    inline void DrawAnatomy(ImVec2 pos, ImVec2 size, int* selected)
    {
        ...
        if (ImGui::IsItemHovered())
        {
            ...
            if (h >= 0)
            {
                static const char* hitNames[] = {"Head","Neck","Chest","Stomach","Pelvis","Left arm","Right arm","Left leg","Right leg"};
                ImGui::SetTooltip("%s", hitNames[h]);
    ...
    inline void PageAimbot()
    {
        ...
        ImGui::Dummy(ImVec2(0,gap));
        float keysH = avail.y-topH-midH-76.f-gap*3.f;
        if (keysH < 60.f) keysH = 60.f;
        CardBegin("keys","KEYBINDS",ImVec2(rightW,keysH),3);
    ...
    inline void PageConfigs()
    {
        ...
        const float btnW=(ImGui::GetContentRegionAvail().x-8.f)/2.f;
        if(ImGui::Button("Save",ImVec2(btnW,34.f)) && name[0]){gui_config::Save(name); configs=gui_config::List();}
        ImGui::SameLine(0,8.f);
        if(ImGui::Button("Load",ImVec2(btnW,34.f)) && name[0]) gui_config::Load(name);
    ...
    inline void PageDashboard()
    {
        ...
        TextMuted("Entity system"); ImGui::SameLine(); ImGui::TextColored(st.entitySystemReady?success:muted,"%s",st.entitySystemReady?"Ready":"Waiting");
        TextMuted("View matrix"); ImGui::SameLine(); ImGui::TextColored(st.viewMatrixReady?success:muted,"%s",st.viewMatrixReady?"Ready":"Waiting");
        CardEnd(); ImGui::SameLine(0,gap);
        ...
        ImGui::TextColored(muted,"Backend"); ImGui::TextColored(Trace::Ready()?success:muted,"%s",Trace::Ready()?"Ready":"Not ready");
        CardEnd();
        ImGui::Dummy(ImVec2(0,gap));
    ...
    inline void Render(float alpha)
    {
        ...
        ImGui::SetNextWindowSizeConstraints(ImVec2(1120.f,780.f),ImVec2(2200.f,1400.f));
        if(centerNext)
        {
        ...
        d->AddRectFilled(wp,ImVec2(wp.x+sideW,wp.y+ws.y),C(sidebar),12.f);
