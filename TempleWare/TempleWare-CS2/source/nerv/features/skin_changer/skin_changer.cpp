...
void c_skin_changer::process_knife(c_econ_entity* weapon, c_econ_item_view* item, c_cs_player_pawn* local_pawn, bool force_update, bool& did_update) {
	...
	static int s_kdbg = 0;
	if ((s_kdbg++ % 120) == 0)
		LOG_INFO(xorstr_("[knife] def=%d selected=%d paintId=%d changed=%d force=%d"),
			(int)def_index, (int)selected_knife, paint_kit_id, (int)config_changed, (int)force_update);

	if (def_index == selected_knife && !config_changed && !force_update)
		return;

	LOG_INFO(xorstr_("[knife] APPLYING def=%d->%d paintId=%d"), (int)def_index, (int)selected_knife, paint_kit_id);
...
void c_skin_changer::run(int stage) {
	...
	static int s_dbg = 0;
	const bool s_log = ((s_dbg++ % 180) == 0); // ~ every 3s
	...
	if (!skin_enabled && !knife_enabled) {
		if (s_log) LOG_INFO(xorstr_("[apply] both disabled (skin=%d knife=%d)"), (int)skin_enabled, (int)knife_enabled);
		should_update = false;
		return;
	}

	if (!g_nctx->m_local_pawn) {
		if (s_log) LOG_INFO(xorstr_("[apply] local_pawn null"));
		return;
	}

	auto* local_pawn = reinterpret_cast<c_cs_player_pawn*>(g_nctx->m_local_pawn);
	if (s_log) LOG_INFO(xorstr_("[apply] pawn=%p valid=%d health=%d"),
		(void*)local_pawn, (int)valid_ptr(local_pawn), local_pawn ? local_pawn->m_health() : -999);
	if (!valid_ptr(local_pawn)) {
		if (s_log) LOG_INFO(xorstr_("[apply] pawn invalid"));
		return;
	}
	// NOTE: health gate temporarily bypassed to isolate whether m_health offset
	// is wrong (it blocks all apply if so).
	// if (local_pawn->m_health() <= 0) return;
	if (s_log) LOG_INFO(xorstr_("[apply] running skin=%d knife=%d knifeSel=%d knifePaint=%d"),
		(int)skin_enabled, (int)knife_enabled, g_cfg->knife_changer.m_knife, g_cfg->knife_changer.m_paint_kit);

	if (local_pawn->m_is_buy_menu_open())
		should_update = true;

	auto* weapon_service = local_pawn->m_weapon_services();
	if (s_log) LOG_INFO(xorstr_("[apply] weapon_service=%p valid=%d"), (void*)weapon_service, (int)valid_ptr(weapon_service));
	if (!valid_ptr(weapon_service))
		return;

	auto& my_weapons   = weapon_service->my_weapons();
	if (s_log) LOG_INFO(xorstr_("[apply] my_weapons size=%u"), (unsigned)my_weapons.m_size);
