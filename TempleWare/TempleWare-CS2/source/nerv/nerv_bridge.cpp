...
namespace nerv_bridge {
	...
	void tick(bool menu_open, void* tw_local_pawn, void* tw_local_controller) {
		...
		static int s_t = 0;
		const bool tlog = ((s_t++ % 180) == 0);
		__try {
			if (!g_interfaces || !g_interfaces->m_entity_system) {
				if (tlog) nerv_log("tick: interfaces/entity_system null");
				return;
			}

			// nerv's get_local_pawn() and TempleWare's g_ctx (set from an
			// unreliable frame hook) both come up null on this build. Read the
			// local pawn/controller straight from client.dll globals instead.
			//   dwLocalPlayerPawn = 0x23C6268, dwLocalPlayerController = 0x23A0F30
			//   (cs2-dumper, 2026-08-30 build).
			const uintptr_t client = reinterpret_cast<uintptr_t>(GetModuleHandleA("client.dll"));
			void* pawn = nullptr;
			void* ctrl = nullptr;
			if (client) {
				pawn = *reinterpret_cast<void**>(client + 0x23C6268);
				ctrl = *reinterpret_cast<void**>(client + 0x23A0F30);
			}
			if (!pawn)  pawn = g_interfaces->m_entity_system->get_local_pawn();
			if (!pawn)  pawn = tw_local_pawn;
			if (!ctrl)  ctrl = tw_local_controller;
			g_nctx->m_local_pawn = pawn;
			g_nctx->m_local_controller = ctrl;
			if (!g_nctx->m_local_pawn) {
				if (tlog) nerv_log("tick: no pawn (client=%p)", (void*)client);
				return; // not in a live match yet
			}

			if (tlog) nerv_log("tick: pawn=%p -> run", (void*)g_nctx->m_local_pawn);
			if (menu_open)
				force_update();
